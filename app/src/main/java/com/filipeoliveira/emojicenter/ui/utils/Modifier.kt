...
fun Modifier.shimmerEffect(
    speed:Int = 1000,
    gradientLight: Color = Color(0xFFB8B5B5),
    gradientDark: Color = Color(0xFF8B8A8A)
) : Modifier = composed {

    var size by remember { mutableStateOf(IntSize.Zero) }

    val translation = rememberInfiniteTransition(label = "")

    val startOffsetX by translation.animateFloat(
        initialValue = -2 * size.width.toFloat(),
        targetValue = 2 *  size.width.toFloat(),
        animationSpec = infiniteRepeatable(animation = tween(speed)), label = ""
    )
